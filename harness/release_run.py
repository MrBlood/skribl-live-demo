...
def release_status(ok, pending, state):
    ...
    AND IT TOOK THE SOURCE STATE LAST, having been written without it. `state`
    was not a parameter, so a run on a DIRTY tree printed

        release status   FULL RELEASE PASS
        source state     DIRTY, --allow-dirty used (...) — NOT A SEALABLE RUN

    two lines apart, in that order, with the strongest label on top. The line
    underneath was right and nobody reads the small print first. The same held
    for `unknown`: a run that could not establish what the tree was still
    claimed the full pass.

    `generated-only dirty` stays sealable, because that IS the process working
    — the mid-run attestation write that source_state() exists to distinguish.
    """
    if not ok:
        return "FAIL"
    # NOT SEALABLE outranks EXTERNAL COVERAGE PENDING: a lane can be attested
    # later, but nothing later makes an unrecorded tree reproducible.
    if state == "dirty":
        return "NOT SEALABLE — WORKING TREE DIRTY"
    if state == "unknown":
        return "NOT SEALABLE — WORKING TREE STATE UNKNOWN"
    if pending:
...
def source_state():
    ...
    # A FILE CAN BE IN THE FROZEN HASH AND IN NO COMMIT, and this said `clean`
    # about a tree like that. The two halves disagreed about what "the tree"
    # means: tree_hash() walks tree_files(), which is `find . -type f` minus a
    # few prefixes, so it hashes whatever is ON DISK — while this asked `git
    # diff --name-only HEAD`, which reports only files git already TRACKS.
    # An untracked file therefore entered the hash without ever reaching this
    # list, and the sealed record claimed a clean tree whose hash cannot be
    # reproduced from the commit it names. That is the whole point of the hash.
    #
    # DERIVED FROM tree_files() RATHER THAN RESTATED, so the two cannot drift.
    # The rule is one sentence: every file in the hash is either tracked and
    # accounted for above, or in GENERATED (which tree_files() already drops).
    # Anything else is source no commit records, which is exactly what the
    # "dirty" half of this function was written to name.
    ls = subprocess.run(["git", "-C", str(ROOT), "ls-files"],
                        capture_output=True, text=True, timeout=30)
    if ls.returncode != 0:
        err = (ls.stderr or "").strip().splitlines()
        return "unknown", [err[-1] if err else f"git ls-files exited {ls.returncode}"]
    tracked = {p for p in ls.stdout.split("\n") if p.strip()}
    untracked = sorted(f for f in tree_files() if f not in tracked)

    if not dirty and not untracked:
        return "clean", []
    source = [p for p in dirty if p not in GENERATED] + untracked
    return ("dirty", sorted(set(source))) if source else ("generated-only dirty", dirty)
...
def main():
    ...
    lines = [
        ...
        f"    release status   {release_status(ok, _pending, state)}",
