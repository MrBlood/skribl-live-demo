...
**Rate-limiter contract, finished (v135).** v134 corrected one of `verify_review.py`'s
THREE concurrency bursts and claimed all of them. A second review caught it. All
three are now consistent; grep for `threading.Thread` before assuming otherwise.

**Rate-limiter contract (v134).** The quota race was never a flake: the limiter
guarantees AT MOST N, not EXACTLY N, and two suites had been asserting exactness
and passing on luck. Corrected to assert the safety property. Also: the v133
onion sub-rows were dimmed but not disabled, so the keyboard could still reach
them.

**Settings drawer (v133).** The drawer became a grouped list — Speed, Canvas,
Onion skin, Pages behind, Tint — on the rule that the ⋯ menu holds verbs and the
drawer holds nouns. Canvas moved in from that menu and became a dropdown; onion
gained a switch that mirrors the header button it deliberately keeps. Measurement
caught a ragged right edge (three controls, three different edges) that a fixed
150px label column had been causing since v129.

**Stylus pressure (v132).** An optional per-point `p` field, following v109's
`hold` rules exactly: only reader is `pointWidth()`, only written by a pen, and a
mouse stroke still serialises to the bytes it always did. The Pad and Flip needed
separate capture paths because they are bound to different event models, and the
...
Sign-off round. The only outstanding item was a stale section heading in
`verify_review.py`:

    R2#2 — post quota is exact, and atomic under concurrency

Neither clause survived v135 intact. The section has two halves that prove
different things, and the heading claimed the stronger one for both. Sequentially
— requests one after another — the quota IS exact, and that is still asserted.
Concurrently it is only *bounded*: never more than the quota, with racers free to
withdraw and admit fewer. And nothing in it is atomic; the whole point of v134
and v135 is that `reserve()` is insert -> commit -> count -> delete-if-over with
no lock anywhere.

Renamed to "post quota is exact SEQUENTIALLY, and bounded under concurrency",
with a comment spelling out which half proves which.

Worth noting what this was. The assertions were corrected in v135 and the heading
above them was not — so a reader skimming for the contract would have taken the
wrong one from the loudest line on the screen. That is the same failure as the
stale `README` claiming v118 for fourteen versions, and the `harness/README`
claiming 615 assertions across 18 suites while a whole suite went unlisted. **A
heading is a claim.** It goes stale exactly like a roadmap item, and it is read
more often than the code beneath it.

No application code changed: `app.py` moved only its version string, no static
file changed, so no `?v=` was bumped. v136 exists so two different archives are
not both called v135.

## v135 — the same correction, finished properly

### I fixed one of three bursts and said I had fixed them all
v134's release note claimed "Both now assert the real contract" and "Both bursts
now assert the property that actually matters". A second review checked and that
was false. `verify_review.py` has **three** concurrency bursts, not one:

- ~line 364 — the base3 burst, 12 SQLite threads against a quota of 2
- ~line 725 — 8 threads against an already-exhausted bucket, must admit zero
- ~line 752 — the fresh-bucket burst, 12 threads

v134 corrected the third, because that is where the failure surfaced. The first
still asserted `out.count(201) == 2` and `count(429) == 10`. So v134 shipped
asserting **both** contracts at once: that admission may be fewer than two, and
that it must be exactly two. The recorded 278/278 meant the obsolete block got
lucky in that particular run — the precise failure mode v134 claimed to have
eliminated, still present, in the file v134 was about.

The lesson is narrow and worth keeping: **I searched where the failure appeared
instead of searching for the pattern.** One `grep -n "threading.Thread"` would
have found all three in v134. It is the same shape of mistake as reading a
screenshot for what I expected rather than what was there.

### What the base3 burst asserts now
Its original comment mattered: round 3 #2 had explicitly rejected a bare "at most
2" as too weak, and replaced it with an exact split **plus a database row delta**.
The row delta was the valuable half — 201s are responses, not rows, and tying
them together is what catches a limiter that admits without persisting. That half
is kept. The exact split is replaced by the real contract: never more than quota,
at least one winner, refusals equal the remainder, no other status, and committed
rows equal successful responses.

Three consecutive runs, 279/279 each, 2×201 / 10×429 every time — passing on the
contract rather than on a coincidence.

The exhausted-bucket burst at ~725 needed no change: asserting zero admissions
from a spent quota is unaffected, since at-most-zero and exactly-zero coincide.

### A test that broke three other tests
The v134 accessibility assertion was named for keyboard operation but called
`.click()`. Making it honest took two failed attempts, both recorded in the file:

1. Driving real Enter/Space through the page — the control correctly refuses
   focus, so the keys reached the document and fired Flip's own shortcuts,
   changing the page and breaking three later assertions.
2. Dispatching a synthetic `keydown` with `bubbles: true` — same failure, and it
   modelled nothing real, because a native disabled button never emits that event
   at all.

Forcing an event the browser would never generate is not a stronger test, it is a
fictional one. What `disabled` guarantees is that the control cannot be focused
and cannot be activated, and that is what is asserted now.

### No application code changed
`app.py` moved only its version string; no static file changed, so **no `?v=`
cache-bust was bumped** — the rule is to bump only what changed. The version
number moved to v135 solely so two different archives are not both called v134.

## v134 — the rate limiter's contract, written down at last

### The retraction, which is the whole round
v133 shipped with a note saying the 12-thread quota race was "load-sensitive
rather than broken". An external review read the code and showed that is wrong.

`_db_rate_reserve_post` is **insert -> commit -> count -> delete-if-over**. There
is no lock. Two racers can each commit their pending row before either performs
its count; both then see a total above quota, and both delete themselves. **A
burst of twelve against a quota of two can legitimately admit one.** That is not
load sensitivity — it is the algorithm being fail-closed, and load only changes
how often you observe it.

So the contract has always been **AT MOST N**, never **EXACTLY N**. I documented
a specification mismatch as a harness flake, and the fix I proposed — a barrier
so the threads fire simultaneously — would have made the symptom rarer while
leaving the mismatch in place. The source comment on `RateEvent` was more honest
than my release note the whole time: it said "biased safe", not proven. I had it
in front of me.

### The worse half, which the review did not reach
Correcting `verify_review.py` meant reading `verify_postgres.py`, and it asserted
the same impossible contract — plus this:

    check("no UNDER-admission: the quota was actually used, not merely blocked",
          created == QUOTA and out.count(201) == QUOTA)

An explicit assertion that the limiter must NOT under-admit, against an
implementation that under-admits by design, across four PROCESSES where the
window between commit and count is wider than it is across threads. That is the
suite that supposedly proves cross-process safety. It, too, had been passing on
luck since v123.

### What the suites assert now
Both bursts now assert the property that actually matters, and assert it hard:

- **never MORE than the quota is admitted** — the safety property; over-admitting
  is a security failure, under-admitting costs one user one post
- at least one request wins, so a limiter that blocked everything would fail
- refusals == total - admitted, so no request is silently lost
- no status other than 201/429

Three consecutive PostgreSQL runs: 2×201 / 10×429 each time, passing on the real
contract rather than on a coincidence.

### Making "exactly N" true, and why it is not built
It needs serialized slot allocation: a transaction-scoped advisory lock keyed on
bucket + key_hash, a counter row taken with `SELECT ... FOR UPDATE`, serializable
transactions with bounded retry, or an atomic Lua script in Redis. Every one of
them serializes all posting for a given client to buy exactness that no product
requirement asks for. `SKRIBL_RATE_MAX_POSTS` is abuse prevention; failing closed
under a burst is the right bias. **Left unbuilt deliberately, and now written
down as a decision rather than left as an accident.** See ROADMAP.

### Dimming is not disabling
The v133 onion sub-rows used `pointer-events: none` with `opacity: .4`. That
stops a mouse and nothing else — a control tabbed to before onion was switched
off keeps focus and still fires on Enter. They now get real `disabled` plus
`aria-disabled`, so "off" means off on every input path and a screen reader
announces the state instead of reading an apparently live control. Asserted,
including that a programmatic click on a disabled depth button does not change
`onionDepth`.

### Two review findings that did not hold
Checked against the code rather than accepted:

- The `console.log('[photo] normalize', ...)` in `app.js` was reported as a stray
  development log. It is already gated behind `window.__SKRIBL_PHOTO_DEBUG` and
  wrapped in try/catch — a deliberate debug hook. Left alone.
- The drawer switch's `aria-checked="true"` was reported as contradicting a
  default of `false`. `flip.js` has `onion = true`; the markup matches the
  default. Left alone.

The review was right about the thing that mattered and wrong about two small
things, which is roughly the ratio to expect. Read the code either way.

## v133 — the settings drawer became one place, and a claim I had to retract

### The retraction first
I opened this round by reporting that the drawer's onion row rendered a label and
a hint with **no control at all**. It does not. `onion` defaults to `true`, so
those controls were on screen in the very screenshot I was describing, and I read
an empty row into it. v129's dim-don't-hide behaviour works exactly as
documented. Acting on that report would have meant fixing a bug that was never
there — which is the same failure mode as trusting a stale roadmap item, except
the stale claim was mine and one screenshot old.

### Screenshots came back, then went away again
They rendered for the first two captures of this round — the first visual
confirmation since v128 — and stopped partway through. Everything after that
point is measured, not seen. The final layout was signed off by the user from
exported PNGs rather than by me, which is the arrangement that actually works
when the render channel is unreliable: **export the images as files instead of
trying to look at them.** Worth doing by default from here.

### What moved, and the rule that decided it
The drawer is now a grouped settings list — one row per setting, name and a
plain-language hint on the left, control flush right. The rule: **the ⋯ menu
holds verbs, the drawer holds nouns.**

- **Speed** — segmented 6/12/24, unchanged.
- **Canvas** — moved IN from the ⋯ menu, and converted from a four-way segmented
  control to a dropdown. Four named presets don't fit as pills at 360px, and a
  native `<select>` gets the platform's own picker on a phone. v110 put it in the
  ⋯ menu reasoning that it is a document property rather than a tool — correct
  about what it is, wrong about where that lands it, because the ⋯ menu was
  otherwise entirely actions.
- **Onion skin** — a switch in the drawer, mirroring the header button. The
  header shortcut was deliberately KEPT: onion gets toggled constantly while
  animating, so it earns its place on the bar. Both views call the same
  `setOnion()`, so they cannot drift — asserted by tapping one and checking the
  other follows.
- **Pages behind** and **Tint by distance** — indented sub-rows under the switch.
  They still DIM rather than hide when onion is off, which is v129's call and
  still the right one: the drawer animates to its content's own height, so hiding
  them would make the panel jump while you toggle. They are now also
  `pointer-events: none` when dim, so a disabled row can't be tapped into a
  confusing state.

The ⋯ menu now holds Save, Load, Export and How Flip works. Nothing else.

**Grid stayed in the Draw drawer**, next to Draw-on. It is a drawing aid paired
with another drawing aid; moving it to pad out the settings list would trade one
coherent group for two scattered ones.

### The alignment bug only measurement could find
`.tune-label` was pinned at `width: 150px`, so every control started at the same
x and **ended wherever its own width landed**: a 51px switch, a 122px dropdown
and a 104px pill gave three different right edges — 223 / 294 / 280 at 390px. iOS
right-aligns a row's control, and the raggedness is the kind of thing that reads
as "slightly wrong" without being locatable by eye. The label now takes the slack
and each control is pushed flush right. Spread is **0.00px at 360, 390 and 768**,
and that spread is asserted rather than eyeballed.

### One TDZ bug, caught before it shipped
The canvas wiring lives around line 1500 and referenced `tuneBtn`, which is
declared `const` at line 2005 — a temporal-dead-zone `ReferenceError` that would
have thrown on load and taken the whole surface with it. Resolved inline instead.
A reminder that in a 2000-line script with top-level `const`, position matters.

### Verification
`verify_pages.py` grew from 38 to 48 assertions: the drawer switch mirrors the
header, both sub-rows dim together, canvas is in the drawer and **gone** from the
⋯ menu, choosing a shape actually resizes the canvas, and the right edges align.
694 assertions across 20 suites, 0 skipped.

## v132 — stylus pressure, and a flake that was never mine

### `p`: the first new payload field since v109's `hold`
A point may now carry `p`, normalised pen pressure in 0..1. It deliberately
reuses the compatibility rules `hold` established, because they are the reason
`hold` cost nothing:

- **`pointWidth()` is the ONLY reader.** It never trusts `p` to exist, and clamps
  NaN, strings, `null`, negatives and >1 back into range. A point without the
  field returns its stored `size` untouched, which is what makes every pre-v132
  Skribl replay identically.
- **`p` is written ONLY by a stylus reporting a non-neutral force.** A mouse or
  finger stroke records no `p` key at all, so its payload is byte-identical to
  what it was before. Asserted against a real browser-drawn stroke — 0 of 12
  mouse points carried the field — not assumed.
- **An older player ignores it** and renders uniform width.

The multiplier is linear and centred so neutral pressure is a no-op: 0.5 → 1.0×,
1.0 → 1.65×, 0.0 → 0.35×, with the width floored at 0.5px so a near-zero sample
thins the line instead of making it disappear.

### The two surfaces needed different capture, and that limits the feature
The Pad is bound to **mouse and touch** events; Flip is bound to **pointer**
events. A `MouseEvent` has no pressure at all, so the Pad reads `Touch.force`
and only when `touchType === 'stylus'`, which is the Apple Pencil path in
WebKit. Flip reads `PointerEvent.pressure` when `pointerType === 'pen'`.

**The consequence is worth stating plainly: the Pad gets Apple Pencil on iOS but
does not get desktop drawing tablets.** A Wacom on a laptop reports pressure
through `PointerEvent` only, which the Pad never sees. Fixing that means
migrating the Pad to pointer events — which also touches pinch-to-zoom and the
window-level mouseup commit, so it was left out rather than folded into a
feature change. It is in ROADMAP.

Live rendering goes through the same `pointWidth()` the replay uses. Drawing the
wet-stroke preview at the unmodified `size` would have made the stroke you draw
differ from the stroke that plays back, which is the kind of divergence the
replay core was factored out to prevent in the first place.

Server-side, `p` is range-checked when present, on the same reasoning already
applied to `size`: a hand-built payload should be refused at the door.

### There is no pen in the sandbox
`verify_pressure.py` is 38 assertions and it is honest about its own ceiling. It
calls `pointWidth()` against the real loaded modules (not a reimplementation, so
it cannot drift from the code it guards), drives a real mouse stroke to prove the
byte-identity rule, and posts in-range and out-of-range payloads in both
directions. What it never exercises is the hardware read itself. **Nobody has
drawn on this with a real stylus.**

### The drawer flake was pre-existing, and v131's green was luck
`verify_pages.py` failed on the first full run after this work, at "collapses to
exactly zero height". The obvious conclusion was that I had broken something. I
had not — but a single clean run on the pristine v131 archive appeared to
confirm that I had, which is exactly the trap the "measure, don't assume" rule
exists for.

Running it repeatedly is what settled it: **v131 fails this assertion three runs
in four.** The drawer transitions `grid-template-rows` over 260ms and the suite
measured after a flat 300ms wait, leaving 40ms of margin that a loaded machine
loses; the reported height was a sub-pixel sliver (0.015625px, 0.375px). It now
waits for `transitionend`. Five consecutive green runs.

So the 38/38 recorded in the v131 archive was a lucky run, not a stable result.
One flat-timeout assertion was enough to make the aggregate unreproducible while
still reporting green — which is the same failure mode that made the aggregate
machine-generated back in v111–v122.

### Docs corrected rather than left to rot
The v131 archive shipped a README claiming **v118** and 50 files, and a harness
README claiming **615 assertions across 18 suites, green as of v122** — which
omitted `verify_postgres.py` entirely, a full suite. Both are fixed. The
handoff also carried a verbatim duplicated v110 heading and two different v109
headings; deduplicated.

One process change: **the docs were written before the recorded harness run this
time.** In v131 they were written after, so the tree hash in `LAST-RUN.txt` could
never match the shipped archive, and reproducing it looked like tampering until
you checked mtimes. Now only `LAST-RUN.txt` and `SHA256SUMS` change after the
run, and both are already excluded from the tree hash — so the hash a reviewer
computes from the archive is the hash in the file.
