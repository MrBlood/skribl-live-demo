...
**This is v223, SEALED.** The full release aggregate HAS been executed against
this tree: `harness/RELEASE.md` is generated from it and records PASS, 3134
assertions, 74/74 suites reporting, 1 skipped (`verify_mp4.py` — no H.264
profile in the build container's Chromium). Both the tree hash and the counts
are computed by `release_run.py`, never typed into prose. Narrative and
technical notes: `V223-CHANGES.md`.
...
**The most useful thing in this release for a reviewer is not a feature.** It is
that `harness/RELEASE.md` had been frozen for several versions describing a
64-suite tree, because `release_run.py` refuses to start when a suite on disk
belongs to no batch and TEN did. Every other document points at RELEASE.md for
volatile facts, so the stale number was the authoritative one, and nothing said
so. The refusal worked exactly as designed; what failed is that writing a suite
requires adding a line to `BATCHES` and only a comment inside the refusing file
said that. It is the third time (v199, v219, v223).

If you want a thread to pull: ask what else in this repo is generated by a
process that can silently stop being run.
...
## Verify before believing any of this, including me
    ...
    cd skribl-v223                            # the name is DERIVED from SKRIBL_VERSION
