...
**This release is a response to an outside review of the v223 archive.** All
eight findings are implemented, each with a regression suite written before or
alongside the fix: media resource limits (a 66-byte PNG declaring 30000x30000
used to pass every check), an operable orphan sweep, four host seams that the
documentation described and the code did not have, and three configuration
defects. `V224-CHANGES.md` maps finding to commit to suite.

**The most useful thing here for a reviewer is not a feature.** It is the shape
the eight findings share, and it is the same shape as the previous release's
...
If you want a thread to pull: ask which other claim in this repo is enforced by
a different artifact than the one that states it.

(The v222 paragraph this replaces: v222 was the durability release —
flush-on-leave, a durability-keyed navigation guard, IndexedDB media drafts,
the P0 prefix navigation fix, and a harness-integrity repair. It shipped
UNSEALED, with the full aggregate not executed against its tree; that is now
history. The v221 paragraph before it: the identity release — header piece
marks, accent token work, tree-hash unification, with a full run behind its
seal. Everything below was written at earlier seals and is kept as history;
...
generated evidence rather than against a literal typed here. The literal was
the same fossil in a different field: this block shipped saying `expect v222`
into a v223 tree, which is precisely the failure the paragraph below describes,
one release after it was written down. This file is
