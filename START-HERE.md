# START HERE — Skribl v199 session primer
...
Verify before believing anything in prose, including this file:
    ...
    cd skribl-v199 && sha256sum -c SHA256SUMS | grep -c ': OK'    # expect 153
...
**Upload `skribl-v199.zip` alongside this file.** Unzip it; it produces a folder
`skribl-v199/`. `git init` goes INSIDE that folder, not above it.
...
<!-- HARNESS-COUNTS -->
**RUN NOT GREEN — 1 suite(s) failed:** verify_player_isolation.py. 56 assertions passed across 2 suite(s) that did report, on sqlite as of v199 (tree `3b9245ccc1cc`).
