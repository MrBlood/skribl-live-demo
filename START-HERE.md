# START HERE — Skribl v191 session primer

## Next session: read this block first

    result       PASS, 1704 assertions, 42/42 suites, 1 declared skip (verify_mp4)
    tree hash    see harness/RELEASE.md — generated, never typed here
    files        133

Verify before believing anything in prose, including this file:

    cd skribl-v191 && sha256sum -c SHA256SUMS | grep -c ': OK'    # expect 135
...
wiring and comments. The target needs a separate player entry point, i.e.
`app.js` stops being the player's file. The AST step that `docs/REFACTOR-v132.md`
...
**Upload `skribl-v191.zip` alongside this file.** Unzip it; it produces a folder
`skribl-v191/`. `git init` goes INSIDE that folder, not above it.
...
<!-- HARNESS-COUNTS -->
**PASS WITH SKIPS — 1693 assertions across 40 suites, none failing, 2 skipped** on sqlite as of v191 (tree `646194b7a938`).
...
**Verify the archive first, in one command:**
    ...
    cd skribl-v191 && sha256sum -c SHA256SUMS | grep -c ': OK'      # expect 135
    # 133, not 125: three suites joined harness/ (deletion_foundation, scrub,
    # player_isolation) and FIVE editor-only bundles came out of app.js
    # (editor_export, editor_post, editor_menu, editor_music, editor_photo).
    # Compare against the manifest, not against this number — the manifest
    # is generated, this line is typed, and typed numbers are what drift.
...
**Comments ship to users.** There is no build step, so `app.js` reaches every
visitor uncompressed and a verbose comment block in a shared path is literally
bytes on the wire. Long reasoning belongs in this file; the code carries a short
warning and a pointer. This was learned by watching the byte ratchet fire on a
refactor whose cost was almost entirely comment text.
